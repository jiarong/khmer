...
###
# usage: bash <thisFile> <PE1> <PE2> <overlapCutOff> <outDir> <tag>
#
...
#use absolute path
PREFIX=$TAG.sga.$ASSEMBLE_OVERLAP
...
# preprocessing to remove ambiguous characters
$SGA_BIN preprocess --pe-mode 1 --phred64 -o $PREFIX.pp $PE1 $PE2
...
#
echo "$SGA_BIN filter --no-kmer-check -t $CPU -p $PREFIX --homopolymer-check --low-complexity-check $SEQ" >> $PREFIX.runLog
(/usr/bin/time -f "eTime: %E; CPU: %P; maxMem: %M; AveMem: %K" \
$SGA_BIN filter --no-kmer-check -t $CPU -p $PREFIX --homopolymer-check --low-complexity-check $SEQ
)

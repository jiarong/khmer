...
#
# preprocess included in this pipeline
...
$SGA_BIN preprocess --pe-mode 0 -o $SEQ.pp $SEQ
...
echo "$SGA_BIN index -a ropebwt -t $CPU -p $PREFIX $SEQ.pp" >> $PREFIX.runLog
(/usr/bin/time -f "eTime: %E; CPU: %P; maxMem: %M; AveMem: %K" \
$SGA_BIN index -a ropebwt -t $CPU -p $PREFIX $SEQ.pp
)
